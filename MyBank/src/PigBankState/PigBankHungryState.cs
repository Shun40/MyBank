...
namespace MyBank.src {
	class PigBankHungryState : PigBankState {
		private int tick;
		private bool isSkip;
		private PigBankView parent;
		public PigBankHungryState(PigBankView parent) {
			this.tick = 0;
			this.isSkip = false;
			this.parent = parent;
		}
		...
		public void drawImage() {
			...
			if(this.isSkip) {
				DX.DrawGraph(Resource.PIGBANK_IMAGE.X, Resource.PIGBANK_IMAGE.Y - 4, Resource.PIGBANK_IMAGE.Handle, DX.TRUE);
			} else {
		...
		public void drawTalking() {
			DX.DrawGraph(Resource.TALKING1_IMAGE.X, Resource.TALKING1_IMAGE.Y, Resource.TALKING1_IMAGE.Handle, DX.TRUE);
		}
		// ブタ貯金箱のマウスオーバー監視
