...
namespace MyBank.src {
	class PigBankOnState : PigBankState {
		private int tick;
		private bool isSkip;
		private PigBankView parent;
		public PigBankOnState(PigBankView parent) {
			this.tick = 0;
			this.isSkip = false;
			this.parent = parent;
		}
		...
		public void drawTalking() {
			DX.DrawGraph(Resource.TALKING4_IMAGE.X, Resource.TALKING4_IMAGE.Y, Resource.TALKING4_IMAGE.Handle, DX.TRUE);
		}
		// ブタ貯金箱のマウスオーバー監視
